...
class CheckbuttonEntry(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.checkbutton = ttk.Label(check_button_frame)
        self.checkbutton.configure(
            text=label,
        )
        # self.checkbutton.configure(
        #     text=label, variable=check_var, command=self.on_click
        # )
        ...
        check_button_frame.pack(side=tk.LEFT, expand=False, fill="x")
        self.entry.pack(side=tk.LEFT, pady=3, ipady=5, expand=False, fill="x")
        if disable_var != None:
        ...
        self.entry.bind(
            "<FocusOut>", lambda e: self.entry.event_generate(focusout_event)
        )
