...
class ProviderTree(ttk.Frame):
    ...
    def __init__(
        self, parent, select_event, name="", disable_var=None, debug_var=None, **kwargs
    ):
        getcontext().prec = 8
        ...
        self.treeview.bind(
            "<Button-1>", lambda event: _on_event(event, self._on_drag_start)
        )
        self.treeview.bind(
            "<ButtonRelease-1>", lambda event: _on_event(event, self._on_drag_release)
        )
    ...
    def _sort_columns(self, col="#3"):
        ...
        def build_sort_index():
            ...
            for iid in tv.get_children(""):
                ...
                _displayed_dict = dict(
                    [
                        (
                            list(all_column_values_dict.keys())[displayed_index],
                            list(all_column_values_dict.values())[displayed_index],
                        )
                        for displayed_index in tv["displaycolumns"]
                    ]
                )
    ...
    def _swap_columns(self, col1, col2):
        ...
        column_label_prior = self.treeview.heading(col1)["text"]
        ...
        display_columns_list[offset_to_displayed_1] = display_columns_list[
            offset_to_displayed_2
        ]
    ...
    def insert(self, recordDict=None, last=False):
        ...
        if not last:
            ...
            for key, value in recordDict.items():
                ...
                if mapped_key != None:
                    if mapped_key in ["cpu (/hr)", "dur (/hr)"]:
                        value = str(
                            remove_exponent(Decimal(str(value)) * Decimal("3600.0"))
                        )
            ...
            self.treeview.insert(
                "", "end", iid=insertionDict["rowId"], values=insertionSequence
            )
        ...
        if last:
            ...
            if measure_longest_name > tv.column("name")["minwidth"]:
                tv.column(
                    "name", minwidth=measure_longest_name, width=measure_longest_name
                )
            # logger.debug(tv.column('name'))
            # logger.debug(tv.column('address'))
