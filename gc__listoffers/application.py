...
class Application(tk.Tk):
    ...
    """
        pipe_parent := input pipe to pipe_child (at self.offerLookup)
        subnetSelected := subnet selected via the gui
    """
    ...
    def lookup_offers(self):
        refresh = True  # revisit
        if refresh:
            self.id += 1
            # clear tree
        ...
        pipemsg = {"id": self.id, "msg": {"subnet-tag": self.subnetSelected, "sql": ss}}
    ...
    def _on_subnet_radio(self, e):
        print(e)
        print(dir(e))
        radio_label = self.variables["subnet_radioVar"].get()
        print(radio_label)
        if radio_label == "other":
            subnetOtherTyped = self.variables["subnet_comboVar"].get()
            print(subnetOtherTyped)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tk.call("source", str(projectdir / "Sun-Valley-ttk-theme/sun-valley.tcl"))
        s = ttk.Style()
        # s.theme_use("forest-light")
        self.tk.call("set_theme", "sun-valley")
        self.variables = Variables()
        self.variables["debug_var"].set(False)
        ...
        self.bind("<<Subnet Radio Pressed>>", self._on_subnet_radio)
        self.variables["signals"]["new_filter_criteria"].trace_add(
            "write", self._refresh_update_button
        )
        ...
        self.pipe_parent, pipe_child = multiprocessing.Pipe()
        self.localConnection = LocalConnection(pipe_child, self.offerLookup)
        self.localConnectionProcess = multiprocessing.Process(
            target=self.localConnection, daemon=False
        )
